...
export interface AuthService {
  ...
  generateMagicLink(email: string): Promise<{ success: boolean; token?: string }>;
  validateMagicLink(token: string): Promise<{ success: boolean; user?: User; sessionToken?: string }>;
  validateSession(sessionToken: string): Promise<{ valid: boolean; user?: User }>;
...
export interface WebSocketService {
  ...
  authenticateConnection(ws: WebSocket, sessionToken: string): Promise<User | null>;
  broadcastMessage(message: StandardMessage, userFilter?: (user: User) => boolean): Promise<void>;
